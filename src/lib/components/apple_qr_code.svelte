<script lang="ts">
    import qrcodeImg from "$lib/assets/images/qr_code.png";
    ...
    let {
        open = $bindable(true),
        appStoreUrl = "https://apps.apple.com/search?term=piviit",
        onclose,
    }: Props = $props();
...
{#if open}
    ...
    >
        ...
        >
            <!-- Header -->
            <div
                class="bg-[#f8f6f2] p-2 flex gap-2.5 items-start border-b border-black/6"
            >
                <div
                ...
                <div class="pt-1 flex-1">
                    ...
                    <div class="flex justify-between flex-wrap">
                        {#each ["Free", "4+", "In-app purchases"] as tag}
                            <span
                                class="text-[11px] font-semibold bg-[#e0dfd9] text-[#444441] rounded-md px-1.5 py-0.5"
                            >
                                {tag}
                            </span>
                        {/each}
